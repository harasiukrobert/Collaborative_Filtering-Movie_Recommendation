...
from dataset_without_genre import MovieLensDataset
...
if __name__ == '__main__':
    for i in range(1,6):
        logger = TensorBoardLogger(save_dir="lightning_logs_no_genre")
        ...
        if os.path.exists("checkpoints_no_genre/"):
            files = [os.path.join('checkpoints_no_genre/', file) for file in os.listdir('checkpoints_no_genre/') if file.startswith('last')]
        ...
        trainer = pl.Trainer(max_epochs=20, fast_dev_run=False, accelerator='gpu', callbacks=[checkpoint_callback],logger=logger)
